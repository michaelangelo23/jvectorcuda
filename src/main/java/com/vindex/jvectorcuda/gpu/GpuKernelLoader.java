...
public class GpuKernelLoader {
    ...
    // Per-context cache for loaded modules (key: context handle -> ptxFileName ->
    // module)
    // Modules are context-dependent and must be cached per-context to avoid
    // INVALID_HANDLE errors
    ...
    public GpuKernelLoader(String ptxFileName, String kernelName) {
        ...
        // Get current context - use hashCode as identifier since getNativePointer() is
        // protected
        ...
        module = moduleCache.computeIfAbsent(ptxFileName, key -> {
            ...
            logger.info("Loaded and cached CUDA module from {} for context {}", ptxFileName,
    ...
    /**
     * Launches a 2D grid kernel for batch processing.
     * 
     * @param gridX            grid dimension in X (e.g., number of queries)
     * @param gridY            grid dimension in Y (e.g., number of vector blocks)
     * @param blockSize        threads per block
     * @param kernelParameters kernel parameters pointer
     */
    public void launch2D(int gridX, int gridY, int blockSize, Pointer kernelParameters) {
        int result = cuLaunchKernel(
                function,
                gridX, gridY, 1, // Grid dimensions (x, y, z)
                blockSize, 1, 1, // Block dimensions (x, y, z)
                0, // Shared memory size
                null, // Stream (null = default stream)
                kernelParameters, // Kernel parameters
        ...
        if (result != CUresult.CUDA_SUCCESS) {
            throw new RuntimeException("2D Kernel launch failed: " + CUresult.stringFor(result));
        }
