...
/**
 * Factory for creating {@link VectorIndex} instances with automatic GPU/CPU
 * detection.
 *
 * <p>
 * This factory provides multiple strategies for creating vector indices:
 * <ul>
 * <li><b>Hybrid (Recommended):</b> Intelligent routing between GPU and CPU
 * based on workload</li>
 * <li><b>Auto:</b> Automatically selects GPU if CUDA is available, falls back
 * to CPU</li>
 ...
 * <li><b>Thread-safe:</b> Wraps any index with
 * {@link ThreadSafeVectorIndex}</li>
 * </ul>
 *
 * <h2>Basic Usage</h2>
 * 
...
public final class VectorIndexFactory {
    ...
    /**
     * Creates a hybrid index with intelligent CPU/GPU routing using Euclidean
     * distance.
     *
     * <p>
     * <b>This is the recommended default for most applications.</b> The hybrid
     * index
     * automatically routes queries to the optimal backend based on workload
     * characteristics:
     * <ul>
    ...
    /**
     * Creates a hybrid index with intelligent CPU/GPU routing using the specified
     * metric.
     *
     * @param dimensions the dimensionality of vectors to be indexed
    ...
    /**
     * Creates a builder for configuring a hybrid index with custom thresholds.
     *
     * <p>
     * Use this when you need to tune the GPU/CPU routing thresholds for your
     * specific hardware or workload:
     * 
     * <pre>{@code
     * VectorIndex index = VectorIndexFactory.hybridBuilder(384)
     ...
     *
     * <p>
     * Default thresholds (validated on GTX 1080 Max-Q):
     * <ul>
     * <li>Batch threshold: 10 queries</li>
     * <li>Vector threshold: 50,000 vectors</li>
     * </ul>
     *
     * @param dimensions the dimensionality of vectors to be indexed
     * @return a HybridVectorIndex.Builder for configuring thresholds
     * @throws IllegalArgumentException if dimensions &le; 0
     * @see HybridVectorIndex.Builder
     */
    public static HybridVectorIndex.Builder hybridBuilder(int dimensions) {
        validateDimensions(dimensions);
        logger.debug("Creating hybrid index builder for {} dimensions", dimensions);
        return HybridVectorIndex.builder(dimensions);
    }
     ...
     *
     * <p>
     * Combines the benefits of hybrid routing with thread-safe concurrent access.
     *
     * @param dimensions the dimensionality of vectors to be indexed
     ...
     *
     * <p>
     * Attempts GPU first (5x+ speedup for batch queries), falls back to CPU
    ...
    /**
     * Creates an index with automatic GPU/CPU detection using the specified
     * distance metric.
     *
     * <p>
     * Attempts GPU first (5x+ speedup for batch queries), falls back to CPU
     ...
     *
     * <p>
     * The CPU implementation uses approximate nearest neighbor search (HNSW),
    ...
    /**
     * Creates a GPU-only index using CUDA brute-force search with Euclidean
     * distance.
     *
     * @param dimensions the dimensionality of vectors to be indexed
     ...
     *
     * <p>
     * The GPU implementation uses exact brute-force search, which is slower
     * for single queries (due to upload overhead) but much faster for batch
     * queries.
     *
     ...
     *
     * <p>
     * Uses {@link java.util.concurrent.locks.ReadWriteLock} for concurrent read
     * access and exclusive write access. Multiple threads can search
     * simultaneously,
    ...
    /**
     * Creates a thread-safe wrapper around an auto-detected index with custom
     * metric.
     *
     * @param dimensions the dimensionality of vectors to be indexed
     * @param metric     the distance metric to use
     ...
     *
     * <p>
     * Recommended for multi-threaded applications using CPU backend.
     *
     * @param dimensions the dimensionality of vectors to be indexed
     ...
     *
     * <p>
     * <b>Note:</b> GPU operations are serialized through CUDA, so thread-safe
